...
export const QuickPaymentDialog: React.FC<QuickPaymentDialogProps> = ({
  ...
  const { data: todayAppointments = [] } = useQuery({
    ...
    queryFn: async (): Promise<AppointmentWithDetails[]> => {
      ...
      return (data || []).map((apt: any) => ({
        ...
        servicePrice: apt.services?.price || 0,
  ...
  const handleSubmit = () => {
    ...
    const paymentData = {
      amount,
      paymentMethod,
      description: description || `Payment from ${selectedClient?.clientName || selectedClient?.name || 'walk-in client'}`,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        {trigger || (
          <Button className="gap-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-xl">
              ...
              {todayAppointments.map(appointment => (
                <Card 
                  ...
                  onClick={() => {
                    ...
                    setAmount(appointment.servicePrice?.toString() || '');
                    setDescription(`Payment for ${appointment.serviceName} - ${appointment.clientName}`);
                  }}
                >
                  <CardContent className="p-4">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <p className="font-bold text-green-600">${appointment.servicePrice}</p>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">
                    Selected: {selectedClient.clientName || selectedClient.name}
                  </span>
                </div>
              </CardContent>
            </Card>
