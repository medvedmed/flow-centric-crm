...
export const WorkingHoursManager: React.FC<WorkingHoursManagerProps> = ({
  ...
  const fetchWorkingHours = async () => {
    ...
    try {
      ...
      const {
        data: staff,
        error
      } = await supabase.from('staff').select('*').eq('salon_id', user.id).eq('status', 'active');
  ...
  const updateWorkingHours = (staffId: string, field: keyof WorkingHours, value: any) => {
    setWorkingHours(prev => prev.map(wh => wh.staffId === staffId ? {
      ...wh,
      [field]: value
    } : wh));
  };
  const toggleWorkingDay = (staffId: string, day: string) => {
    setWorkingHours(prev => prev.map(wh => {
      if (wh.staffId === staffId) {
        const newDays = wh.workingDays.includes(day) ? wh.workingDays.filter(d => d !== day) : [...wh.workingDays, day];
        return {
          ...wh,
          workingDays: newDays
        };
      ...
      <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <CardContent>
          {loading ? <div className="flex items-center justify-center py-8">
              ...
              {workingHours.map((wh, index) => <Card key={wh.staffId} className="border-2">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="outline" size="sm" onClick={() => resetToDefault(wh.staffId)}>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input id={`start-${wh.staffId}`} type="time" value={wh.workingHoursStart} onChange={e => updateWorkingHours(wh.staffId, 'workingHoursStart', e.target.value)} />
                      </div>
                      <div>
                        <Label htmlFor={`end-${wh.staffId}`}>End Time</Label>
                        <Input id={`end-${wh.staffId}`} type="time" value={wh.workingHoursEnd} onChange={e => updateWorkingHours(wh.staffId, 'workingHoursEnd', e.target.value)} />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input id={`break-start-${wh.staffId}`} type="time" value={wh.breakStart} onChange={e => updateWorkingHours(wh.staffId, 'breakStart', e.target.value)} />
                      </div>
                      <div>
                        <Label htmlFor={`break-end-${wh.staffId}`}>Break End</Label>
                        <Input id={`break-end-${wh.staffId}`} type="time" value={wh.breakEnd} onChange={e => updateWorkingHours(wh.staffId, 'breakEnd', e.target.value)} />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {daysOfWeek.map(day => <Badge key={day} variant={wh.workingDays.includes(day) ? "default" : "outline"} className="cursor-pointer" onClick={() => toggleWorkingDay(wh.staffId, day)}>
                            {day}
                          </Badge>)}
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <p className="text-sm text-gray-600">
                        ...
                        {wh.breakStart && wh.breakEnd && <span> (Break: {wh.breakStart} - {wh.breakEnd})</span>}
                      </p>
