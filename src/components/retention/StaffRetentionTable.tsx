...
export const StaffRetentionTable: React.FC<StaffRetentionTableProps> = ({
  ...
  const filteredMetrics = selectedStaffId 
    ? staffMetrics?.filter(staff => staff.staff_id === selectedStaffId)
    : staffMetrics;

  const getRetentionBadge = (rate: number) => {
    if (rate >= 70) return { variant: 'default' as const, className: 'bg-green-600 text-white', text: 'Excellent' };
    if (rate >= 50) return { variant: 'secondary' as const, className: 'bg-orange-600 text-white', text: 'Good' };
    return { variant: 'destructive' as const, className: 'bg-red-600 text-white', text: 'Needs Improvement' };
  };
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredMetrics?.map((staff) => {
                ...
                return (
                  <tr key={staff.staff_id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                    ...
                    <td className="text-center py-4 px-2">
                      <span className="text-white font-bold">{staff.total_unique_clients}</span>
                    </td>
                    <td className="text-center py-4 px-2">
                      <div className="flex flex-col items-center">
                        <span className="text-green-400 font-medium">{staff.new_clients}</span>
                        <span className="text-xs text-gray-500">
                          {staff.total_unique_clients > 0 
                            ? ((staff.new_clients / staff.total_unique_clients) * 100).toFixed(0) + '%'
                            : '0%'
                          }
                        </span>
                      </div>
                    ...
                    <td className="text-center py-4 px-2">
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-xs text-gray-500">
                          {staff.total_unique_clients > 0 
                            ? ((staff.returning_clients / staff.total_unique_clients) * 100).toFixed(0) + '%'
                            : '0%'
                          }
                        </span>
                      </div>
                    ...
                    <td className="text-center py-4 px-2">
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-xs text-gray-500">
                          {staff.total_unique_clients > 0 
                            ? ((staff.loyal_clients / staff.total_unique_clients) * 100).toFixed(0) + '%'
                            : '0%'
                          }
                        </span>
                      </div>
