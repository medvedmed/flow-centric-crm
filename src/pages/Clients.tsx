...
const Clients = () => {
  ...
  // Fetch stats: total, new this month, VIP
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const newThisMonth =
        allClients?.filter(c => c.created_at >= firstOfMonth).length || 0;
      const vipClients =
        allClients?.filter(c => Number(c.total_spent) >= 500).length || 0;

      return { totalClients, newThisMonth, vipClients };
    },
    enabled: !!user?.id,
  });
  ...
  const statsData = [
    ...
    {
      title: 'VIP Clients',
      value: stats?.vipClients?.toString() || '0',
      icon: Star,
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {statsData.map((stat, idx) => (
            <Card
              key={idx}
              className="bg-white/70 backdrop-blur-sm border-violet-200 shadow-lg hover:shadow-xl transition-all duration-300"
            >
          ...
          <CardContent className="p-0">
            {clients.length > 0 ? (
              clients.map(client => (
                <div
                  key={client.id}
                  className="p-6 border-b border-violet-100 last:border-b-0 hover:bg-violet-50/50 transition-colors cursor-pointer"
                >
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="text-center">
                        <p className="text-2xl font-bold text-green-600">
                          ${Number(client.total_spent || 0).toFixed(0)}
                        </p>
